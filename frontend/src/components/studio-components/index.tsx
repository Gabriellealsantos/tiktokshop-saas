    ...
    >
      ...
      >
        {!imgError ? (
          ...
          <div
            className="h-full w-full flex items-center justify-center transition-transform duration-500 group-hover:scale-105"
...
export function OptionTextCard({
  ...
  return (
    ...
    >
      ...
      <div>
        <div className="font-semibold text-text-1 text-[13.5px] tracking-tight mb-0.5">
          {title}
        </div>
        <div className="text-[12.5px] text-white/55 line-clamp-1 leading-snug">
          {description}
        </div>
      </div>
      {selected && (
...
function Choice({
  ...
  return (
    <div className="flex flex-col">
      <p className="mb-2.5 text-[11px] font-bold uppercase tracking-widest text-white/50">
        {label}
      </p>
      <div
        className="flex rounded-full bg-white/[0.04] border border-white/[0.08] p-1.5"
        role="radiogroup"
      >
        {options.map((option) => {
...
export function TakeEditor({ count = 1 }: { count?: number }) {
  const [texts, setTexts] = useState<string[]>([
    "Você ainda está ignorando o detalhe que muda tudo neste produto.",
  ]);
          ...
          <div className="mb-2 flex items-center justify-between">
            ...
            <span
              className={cn(
                ...
                text.split(" ").filter(Boolean).length > 20
                  ? "text-danger"
                  : "text-text-3",
              )}
            >
      ...
      <div className="glass-surface p-6">
        ...
        <div className="mt-4 flex flex-wrap gap-2">
          ...
          <Button
            variant="secondary"
            onClick={() => {
              /* TODO: definir destino do Nano Banana */
            }}
          >
            <ExternalLink />
            Abrir Nano Banana
