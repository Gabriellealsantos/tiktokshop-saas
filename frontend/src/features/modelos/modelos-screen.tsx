...
export function SharedVideoCard({ modelo, isPicker, productId }: { modelo: typeof modelosPlaceholder[0], isPicker?: boolean, productId?: string }) {
  ...
  const handleClick = () => {
    if (isPicker) {
      navigate({ to: "/modelos/usar", search: { id: modelo.id, video: modelo.video, productId } });
    }
  };
  ...
  return (
    <div 
      ...
      className={cn(
        "group relative overflow-hidden rounded-[20px] bg-gradient-to-br from-surface-3 to-deep border border-white/5 transition-all focus-within:ring-2 focus-within:ring-violet-500/50 aspect-[9/16]",
        isPicker 
          ? "hover:border-violet-500/50 hover:-translate-y-1 hover:shadow-[0_12px_24px_-12px_rgba(139,92,246,0.4)] cursor-pointer block w-full text-left"
  ...
  return (
    <AppShell>
      <Page>
        <PageHeader
          eyebrow={
            <div className="inline-flex items-center gap-1.5 px-2.5 py-1 rounded-full bg-violet-500/15 border border-violet-500/20 text-violet-300 text-[11px] font-bold uppercase tracking-[.15em]">
          ...
          title={
            <span className="flex items-center gap-3">
              <span className="flex size-10 shrink-0 items-center justify-center rounded-xl bg-violet-500/15 text-violet-400">
            ...
            {CATEGORIAS.map(cat => {
              ...
              return (
                <button
                  ...
                  className={cn(
                    "px-4 py-1.5 rounded-full text-sm font-medium transition-all duration-200 outline-none focus-visible:ring-2 focus-visible:ring-violet-500",
                    ativo
