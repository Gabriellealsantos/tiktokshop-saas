...
import { avatars as initialAvatars } from "@/mock/data";
        ...
        <PageHeader
          ...
          actions={
            <div className="flex flex-col sm:flex-row gap-3">
              ...
              <Link to="/criar-avatar" className="focus-visible:outline-none">
                <Button>
                  <Sparkles />
                  Criar Avatar
                </Button>
        ...
        <div className="grid gap-4 grid-cols-2 md:grid-cols-3 lg:grid-cols-4 xl:grid-cols-5 mt-6">
          {allAvatars.length === 0 && customAvatars.length === 0 ? (
            ...
            allAvatars.map((avatar) => (
              <div
                key={avatar.id}
                className="group relative overflow-hidden rounded-[20px] bg-surface-2 border border-white/5 transition-all hover:border-white/10 hover:-translate-y-1 hover:shadow-[0_12px_24px_-12px_rgba(139,92,246,0.3)] focus-within:ring-2 focus-within:ring-violet-500/50 aspect-[3/4]"
              >
                <img
                  src={avatar.image}
                  alt={avatar.name}
          ...
          <div
            ...
            className={cn(
              "group flex aspect-[3/4] cursor-pointer flex-col items-center justify-center gap-3 rounded-[20px] border-2 border-dashed transition-all hover:-translate-y-1 focus:outline-none focus:ring-2 focus:ring-violet-500/50",
              isDragging
                ? "border-violet-400 bg-violet-500/10 shadow-[0_0_24px_rgba(139,92,246,0.2)]"
                : "border-white/10 bg-white/5 hover:border-violet-500/50 hover:bg-white/10 hover:shadow-[0_12px_24px_-12px_rgba(139,92,246,0.15)]"
            )}
          >
            <div className="grid size-12 place-items-center rounded-full bg-white/5 text-violet-400 transition-transform group-hover:scale-110 group-hover:bg-violet-500/20">
