...
export function PoseScreen() {
  ...
  const search = getRouteApi("/create-from-scratch/pose").useSearch();
        ...
        <div className="mb-10 grid gap-3 sm:gap-4 sm:grid-cols-2 lg:grid-cols-4">
          ...
          <div className="flex items-center gap-3 rounded-2xl glass-surface p-4 border border-white/10">
            ...
            <div className="min-w-0 flex-1">
              ...
              <p className="text-sm font-medium text-text-1 truncate">{local} · {timeOfDay} · {lighting} · {atmosphere}</p>
            </div>
          </div>
        </div>
        ...
        <div className="flex items-center justify-between border-t border-white/10 pt-6">
          ...
          <Button
            onClick={() => {
              ...
              navigate({
                to: "/create-from-scratch/speech",
                search: (prev: Record<string, unknown>) => ({
                  ...prev,
                  pose: poseSelecionada ?? undefined,
                  manualPose: manualPoseText,
                })
              }).catch(() => {});
            }}
          >
