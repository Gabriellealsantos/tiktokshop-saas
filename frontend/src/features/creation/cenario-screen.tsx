...
import { Page, SectionTitle } from "@/components/base/page";
...
const ILUMINACAO = [
  ...
  { id: "neon", label: "Neon/colorida", icon: Sparkles, gradient: "from-purple-500/30 via-blue-500/20 to-pink-500/30", iconColor: "text-pink-400" },
...
export function CenarioScreen() {
  ...
  const [horarioSelecionado, setHorarioSelecionado] = useState<string | null>(search.horario || null);
  const [iluminacaoSelecionada, setIluminacaoSelecionada] = useState<string | null>(search.iluminacao || null);
  const [atmosferaSelecionada, setAtmosferaSelecionada] = useState<string | null>(search.atmosfera || null);

  const isFormComplete = localSelecionado && horarioSelecionado && iluminacaoSelecionada && atmosferaSelecionada;
    ...
    <AppShell>
      <Page>
              ...
              return (
                <button
                  ...
                  className={cn(
                    ...
                    isSelected
                      ? "ring-2 ring-violet-500 shadow-[0_0_24px_-4px_rgba(139,92,246,0.5)]"
        ...
        <div className="mb-10">
          ...
          <div className="grid gap-2.5 sm:gap-3 grid-cols-2 sm:grid-cols-3 md:grid-cols-5">
            {HORARIOS.map((horario) => {
              const isSelected = horarioSelecionado === horario.id;
              ...
              return (
                <button
                  ...
                  className={cn(
                    ...
                    isSelected
                      ? "ring-2 ring-violet-500 shadow-[0_0_24px_-4px_rgba(139,92,246,0.4)]"
                      : "ring-1 ring-white/10 hover:ring-white/20 hover:-translate-y-1 hover:shadow-lg"
                  )}
                >
        ...
        <div className="mb-10">
          ...
          <div className="grid gap-2.5 sm:gap-3 grid-cols-2 sm:grid-cols-3 md:grid-cols-5">
            {ILUMINACAO.map((item) => {
              const isSelected = iluminacaoSelecionada === item.id;
              const Icon = item.icon;
              
              return (
                <button
                  key={item.id}
                  onClick={() => setIluminacaoSelecionada(item.id)}
                  className={cn(
                    "group relative overflow-hidden rounded-2xl aspect-[16/10] flex flex-col items-center justify-center gap-2 transition-all duration-300",
                ...
                >
                  ...
                  {isSelected && (
                    <>
                      <div className="absolute inset-0 border-2 border-violet-500 rounded-2xl pointer-events-none" />
                      <div className="absolute right-2 top-2 grid size-5 place-items-center rounded-full bg-violet-500 text-white shadow-sm z-10">
                        <Check className="size-3" />
                      </div>
                    </>
                  )}
        ...
        <div className="mb-10">
          ...
          <div className="grid gap-2.5 sm:gap-3 grid-cols-2 sm:grid-cols-4">
            {ATMOSFERA.map((item) => {
              const isSelected = atmosferaSelecionada === item.id;
              const Icon = item.icon;
              
              return (
                <button
                  key={item.id}
                  onClick={() => setAtmosferaSelecionada(item.id)}
                  className={cn(
                    "group relative overflow-hidden rounded-2xl aspect-[16/10] flex flex-col items-center justify-center gap-2 transition-all duration-300",
                    `bg-gradient-to-br ${item.gradient}`,
                    isSelected
        ...
        <div className="mb-6 rounded-2xl glass-surface p-4 border border-white/10 flex flex-wrap items-center gap-2 text-sm text-text-2">
          ...
          <span>{horarioSelecionado ? HORARIOS.find(h => h.id === horarioSelecionado)?.label : "-"}</span>
          <span className="text-white/20">|</span>
          <span>{iluminacaoSelecionada ? ILUMINACAO.find(i => i.id === iluminacaoSelecionada)?.label : "-"}</span>
          <span className="text-white/20">|</span>
          <span>{atmosferaSelecionada ? ATMOSFERA.find(a => a.id === atmosferaSelecionada)?.label : "-"}</span>
        </div>
        ...
        <div className="flex items-center justify-between border-t border-white/10 pt-6">
          ...
          <Button
            ...
            onClick={() => {
              ...
              navigate({
                to: "/criar-do-zero/pose" as any,
                search: (prev: any) => ({
                  ...prev,
                  local: localSelecionado,
                  horario: horarioSelecionado,
                  iluminacao: iluminacaoSelecionada,
                  atmosfera: atmosferaSelecionada
                })
