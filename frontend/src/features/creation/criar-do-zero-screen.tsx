...
export function CriarDoZeroScreen() {
  const navigate = useNavigate();
  const search = useSearch({ strict: false }) as any;
  const [modoAplicacao, setModoAplicacao] = useState(search.modoAplicacao || MOCK_MODOS_APLICACAO[0].id);
  const [somenteNeutros, setSomenteNeutros] = useState(false);
  const [avatarSelecionado, setAvatarSelecionado] = useState<number | null>(search.avatarId ? Number(search.avatarId) : null);
    ...
    <AppShell>
      <Page>
        ...
        <div className="mb-10 rounded-[20px] glass-surface p-5 sm:p-6 border border-white/10">
          ...
          <div className="grid gap-3 sm:grid-cols-2 mb-6">
            {MOCK_MODOS_APLICACAO.map((modo) => {
              const isSelected = modoAplicacao === modo.id;
              return (
                <button
                  key={modo.id}
                  onClick={() => setModoAplicacao(modo.id)}
                  className={cn(
                    "relative flex flex-col items-start gap-1 p-4 rounded-2xl border text-left transition-all duration-300",
                    isSelected
                      ? "bg-violet-500/10 border-violet-500 shadow-[0_0_20px_-4px_rgba(139,92,246,0.2)]"
          ...
          <div className="flex flex-wrap items-center justify-between gap-4 pt-5 border-t border-white/5">
            ...
            >
              <Wand className="size-4 text-violet-400" />
        ...
        <div className="flex items-center justify-between border-t border-white/10 pt-6">
          <Button 
            variant="ghost"
            onClick={() => navigate({ to: "/" })} // Voltando ao suposto modal inicial (Home)
