...
export function CreateFromScratchScreen() {
  const navigate = useNavigate();
  const search = useSearch({ strict: false }) as any;
  const [applicationMode, setApplicationMode] = useState(search.applicationMode || MOCK_MODOS_APLICACAO[0].id);
      ...
      <Page>
        ...
        <div className="flex items-center justify-between border-t border-white/10 pt-6">
          ...
          <Button
            ...
            onClick={() => {
              navigate({
                to: "/create-from-scratch/scenario",
                search: (prev: any) => ({
                  ...prev,
