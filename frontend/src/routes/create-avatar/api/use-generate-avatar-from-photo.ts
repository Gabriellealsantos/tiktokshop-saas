...
export function useGenerateAvatarFromPhoto() {
    ...
    return useMutation({
        mutationFn: async (data: AvatarPhotoFormValues) => {
            const photoResponse = await uploadAvatarReference(await downscaleImage(data.fotoPrincipal as File));
            ...
            if (isUpload) {
                const clothingResponse = await uploadAvatarReference(await downscaleImage(data.fotoRoupa as File));
