...
export function TabRenderizacao({
  form,
}: {
  ...
  const renderSummaryPill = (key: string, val: unknown) => {
    ...
    switch (key) {
      ...
      case "formatoRosto":
        label = "Rosto";
        type = "image";
        image = getRostoImg(String(val));
        break;
      case "tipoFisico":
        label = "Corpo";
        type = "image";
        image = getCorpoImg(String(val));
        break;
      case "estiloCabelo":
        label = "Cabelo";
        type = "image";
        image = estiloCabeloOptions.find((o) => o.name === val)?.image || "";
        break;
      case "roupa":
        label = "Estilo";
        type = "image";
        image = getRoupaImg(String(val));
        break;
      case "etnia":
        label = "Etnia";
        type = "image";
        image = etniaOptions.find((o) => o.name === val)?.image || "";
        break;
      case "expressao":
        label = "Expressão";
        type = "image";
        image = expressaoOptions.find((o) => o.name === val)?.image || "";
        break;
      case "pelosFaciais":
        label = "Pelos";
        type = "image";
        image = pelosFaciaisOptions.find((o) => o.name === val)?.image || "";
        break;
      case "boca":
        label = "Boca";
        type = "image";
        image =
          bocaOptions.find((o) => o.name === val || o.label === val)?.image ||
          "";
        break;
      case "sobrancelha":
        label = "Sobrancelha";
        type = "image";
        image = sobrancelhaOptions.find((o) => o.name === val)?.image || "";
        break;
      case "nariz":
        label = "Nariz";
        type = "image";
        image =
          narizOptions.find((o) => o.name === val || o.label === val)?.image ||
          "";
        break;
      case "corOlhos":
        label = "Olhos";
        type = "image";
        image =
          corOlhosOptions.find((o) => o.name === val || o.label === val)
            ?.image || "";
        break;
      case "tomPele":
        label = "Pele";
        type = "image";
        image = tomPeleOptions.find((o) => o.name === val)?.image || "";
        break;
      case "corCabelo": {
        label = "Cor cabelo";
        type = "image";
    ...
    return (
      <div
        key={key}
        className="inline-flex items-center gap-2 rounded-full border border-brand-500/20 bg-surface-2 pr-3 pl-1 py-1 shadow-sm"
      >
        {type === "image" && image && (
          <img
            src={image}
                ...
                {SUGESTOES_EXTRAS.map((sugestao, idx) => {
                  ...
                  return (
                    <button
                      ...
                      type="button"
                      onClick={() =>
                        form.setValue("detalhesExtras", sugestao, {
                          shouldValidate: true,
                          shouldDirty: true,
                        })
                      }
