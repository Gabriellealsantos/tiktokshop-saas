...
import { uploadAvatarReference } from "@/services/avatarService";

function CardGeracaoAutomatica({
  value,
  onChange,
}: {
  value: string;
  ...
  return (
    ...
    >
      ...
      <div className="absolute inset-0 pointer-events-none overflow-hidden">
        <motion.div
          className="absolute size-1.5 rounded-full bg-white/70 top-[35%] left-[25%]"
          animate={
            shouldReduceMotion
              ? {}
              : { opacity: [0.1, 0.8, 0.1], scale: [0.8, 1.2, 0.8] }
          }
          transition={{ duration: 4, repeat: Infinity, ease: "easeInOut" }}
        />
        <motion.div
          className="absolute size-2 rounded-full bg-brand-400/80 top-[40%] right-[25%]"
          animate={
            shouldReduceMotion
              ? {}
              : { opacity: [0.2, 1, 0.2], scale: [0.9, 1.3, 0.9] }
          }
          transition={{
            duration: 5,
            delay: 1.5,
        ...
        />
        <motion.div
          className="absolute size-1 rounded-full bg-white/90 top-[60%] left-[45%]"
          animate={
            shouldReduceMotion
              ? {}
              : { opacity: [0.1, 0.6, 0.1], scale: [1, 1.5, 1] }
          }
          transition={{
            duration: 3.5,
            delay: 0.5,
      ...
      <div className="absolute inset-x-0 bottom-0 px-3 pb-[10px] pt-6 flex flex-col justify-end pointer-events-none z-10">
        ...
        <p className="text-[10px] text-white/70 font-medium truncate mt-0.5 drop-shadow-sm leading-tight">
          Deixe a IA escolher
        </p>
      </div>
...
function CardEnviarImagem({
  ...
  const handleFile = async (file: File | undefined | null) => {
    ...
    if (!validTypes.includes(file.type)) {
      toast.error("Formato inválido", {
        description: "Por favor, envie apenas imagens PNG, JPG ou WEBP.",
      });
      return;
    }
    ...
    setUploading(true);
    try {
      const response = await uploadAvatarReference(file);
      onUploaded(response.data.url as string);
    } catch {
      toast.error("Falha ao enviar a imagem da roupa.");
    } finally {
      setUploading(false);
    }
  };
  ...
  return (
    <div
      ...
      className={cn(
        "group relative overflow-hidden rounded-[20px] text-left transition-all duration-300 outline-none focus-visible:ring-2 focus-visible:ring-brand-500 aspect-[4/5] bg-surface-2 cursor-pointer",
        preview
          ? isSelected
            ? "ring-2 ring-inset ring-brand-500 shadow-[0_0_24px_-4px_rgba(75,68,232,0.4)]"
            : "ring-1 ring-inset ring-white/10 hover:ring-white/20 hover:-translate-y-1 hover:shadow-[0_12px_30px_-10px_rgba(75,68,232,0.3)]"
        ...
        <>
          ...
          <div
            className={cn(
              ...
              isDragging
                ? "bg-brand-500/60 opacity-100"
                : "bg-brand-500/40 opacity-40 group-hover:opacity-70",
            )}
          />
          <div
            className={cn(
              ...
              isDragging
                ? "bg-brand-500/50 opacity-100"
                : "bg-blue-500/30 opacity-30 group-hover:opacity-60",
            )}
          />
          ...
          <div className="absolute inset-0 flex items-center justify-center pointer-events-none overflow-hidden pb-4">
            <Upload
              className="size-16 text-white/[0.04] transition-transform duration-700 ease-out group-hover:-translate-y-4"
              strokeWidth={1.5}
            />
          </div>
          ...
          <div className="absolute inset-x-0 bottom-0 px-3 pb-[10px] pt-6 flex flex-col justify-end pointer-events-none z-10">
            ...
            <p className="text-[10px] text-white/70 font-medium truncate mt-0.5 drop-shadow-sm leading-tight">
              PNG, JPG ou WEBP &middot; até 5 MB
            </p>
          </div>
        </>
      )}
...
export function TabEstilo({ form }: { form: UseFormReturn<AvatarFormValues> }) {
  ...
  const dynamicRoupaOptions = [
    {
      name: "Casual",
      image: isMasculino ? "/casual-masc.jpeg" : "/casual-fem.png",
    },
    { name: "Luxo", image: isMasculino ? "/luxo-masc.jpeg" : "/luxo-fem.png" },
    {
      name: "Streetwear",
      image: isMasculino ? "/streetwear-masc.jpeg" : "/streetwear-fem.png",
    },
    {
      name: "Fitness",
      image: isMasculino ? "/fitness-masc.jpeg" : "/fitness-fem.png",
    },
    {
      name: "Corporativo",
      image: isMasculino ? "/corporativo-masc.jpeg" : "/corporativo-fem.png",
    },
  ];
  ...
  const sugestoes = estiloSelecionado
    ? roupaSugestoes[estiloSelecionado]
    : undefined;
  ...
  return (
    <div className="space-y-8 animate-in fade-in slide-in-from-bottom-4 duration-500">
            ...
            <FormControl>
              ...
              >
                ...
                <CardEnviarImagem
                  ...
                  onChange={field.onChange}
                  onUploaded={(url) => {
                    form.setValue("clothingImageUrl", url, {
                      shouldValidate: true,
                    });
                    if (!url) form.setValue("clothingPart", undefined);
                  }}
                />
              </ImageOptionSelector>
            </FormControl>
      ...
      {estiloSelecionado === "Enviar Imagem" && (
        <FormField
          control={form.control}
          name="clothingPart"
          render={({ field }) => (
              ...
              <FormControl>
                <div className="grid grid-cols-3 gap-3">
                  {(
                    ...
                    const isSelected = field.value === parte;
                    return (
                      <button
                        key={parte}
                        type="button"
                        onClick={() => field.onChange(parte)}
                        className={cn(
                          "px-4 py-3 rounded-xl border text-sm font-medium transition-all duration-300",
                          isSelected
                            ? "bg-brand-500/10 border-brand-500 text-brand-300 shadow-[0_0_20px_-4px_rgba(75,68,232,0.2)]"
        ...
        render={({ field }) => (
          <FormItem>
            <div className="flex justify-between items-end mb-2">
              <FormLabel className="text-text-2 text-xs font-bold uppercase tracking-wider mb-0 block">
            ...
            <AnimatePresence mode="popLayout">
              {sugestoes && (
                ...
                >
                  <p className="text-[10px] font-bold uppercase tracking-widest text-text-3 mb-2.5 ml-1">
                    Inspire-se
                  </p>
                    ...
                    {sugestoes.map((sugestao, idx) => {
                      ...
                      return (
                        <button
                          ...
                          type="button"
                          onClick={() =>
                            form.setValue("detalhesRoupa", sugestao, {
                              shouldValidate: true,
                              shouldDirty: true,
                            })
                          }
