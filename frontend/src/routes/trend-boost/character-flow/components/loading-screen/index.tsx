...
import { GlassPanel, Page } from "@/components";
...
export function LoadingScreen({ character, loadingStage }: LoadingScreenProps) {
  ...
  useEffect(() => {
    ...
    const duration = 5000; // Tempo estimado (Trend AI stages are faster)
  ...
  const timeRemaining = Math.max(0.1, ((100 - progress) / 100) * 5.0).toFixed(1);
