...
export default function RouteComponent() {
  ...
  return (
    <AppShell>
      <Page className="pt-0 -mt-6 pb-20 max-w-[1400px] mx-auto px-4 md:px-6">
        <div className="rounded-[24px] bg-[#0F0D15]/20 backdrop-blur-3xl border border-white/5 py-5 px-6 mb-6">
          <Link
            to="/trend-boost"
            className="inline-flex items-center gap-1.5 text-sm font-medium text-text-2 hover:text-white transition-colors mb-4"
          >
        ...
        <div className="flex flex-col gap-6">
          {groupBySubcategory(template.characters).map(([subcategory, chars]) => (
            <div key={subcategory || "outros"} className="rounded-[24px] bg-[#0F0D15]/20 backdrop-blur-3xl border border-white/5 p-6">
              <section>
                {subcategory && (
